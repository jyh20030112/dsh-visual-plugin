...
function apply(ctx) {
	...
	*/
	...
	async function describeImagesInMessage(message, facts, signal) {
		...
		return {
			text: (await describeImage(facts.url, facts.apiKey, facts.model, toBase64(stored.data), ref.mediaType, visionPromptFor(userTextOf(message)), signal)).description,
	...
	if (systemPrompt !== void 0) systemPrompt.section({
		name: "vision-bridge",
		order: 115,
		text: "The user may attach images. Each attached image is described before it reaches you: you see \"[视觉描述] <description>\\n[附件] <attachmentId>\" (or \"<image attachmentId=\\\"…\\\">\" as a fallback). The [视觉描述] block is already the result of a completed vision call, so answer the current message directly from it. Do not call vision_describe in the same turn that introduces the image. Only call it for a later user question when the existing description lacks the requested detail."
	});
