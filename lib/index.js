...
function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (ref === void 0) throw new Error(`vision.describe: unknown attachment ${args.attachmentId}`);
			const facts = await resolvedFacts();
			if (facts === void 0) throw new Error("vision.describe: vision model is not configured");
			if (attachments === void 0) throw new Error("vision.describe: attachment service is unavailable");
	...
	if (systemPrompt !== void 0) systemPrompt.section({
		name: "vision-bridge",
		order: 115,
		text: "The user may attach images. Each attached image is described before it reaches you: you see \"[视觉描述] <description>\\n[附件] <attachmentId>\" (or \"<image attachmentId=\\\"…\\\">\" as a fallback). To answer a follow-up question about a specific image, call the vision.describe tool with that attachmentId."
	});
