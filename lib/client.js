		...
		*/
		function VisionBridgePanel(props) {
			...
			const loadConfig = (0, react.useCallback)(async () => {
				...
				const value = resultValue(await api.settings.describe({}));
				if (value === void 0) return;
				const sectionValue = value.namespaces.find((ns) => ns.ns === NS$1)?.value ?? {};
				...
				const credsValue = resultValue(await api.credentials.describe({ refs: [apiKeyEnv] }));
				if (credsValue !== void 0) setKeyConfigured(credsValue.credentials[apiKeyEnv]?.configured ?? false);
			}, [api]);
			/** Save the form: url/model through settings, the key through credentials. */
			const save = (0, react.useCallback)(async () => {
				if (api === void 0) return;
				const apiKeyEnv = (resultValue(await api.settings.describe({}))?.namespaces.find((ns) => ns.ns === NS$1)?.value ?? {}).apiKeyEnv || "VISION_API_KEY";
				if (resultValue(await api.settings.update({
					ns: NS$1,
					patch: {
						url,
						model
				...
				if (apiKey.length > 0) {
					if (resultValue(await api.credentials.set({
						ref: apiKeyEnv,
						value: apiKey
					})) !== void 0) {
