window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		//#endregion
		//#region src/client/vision-bridge-form.ts
		/** Section field names this form stages (url/model are section fields). */
		const SECTION_FIELDS = ["url", "model"];
		/**
		...
		*/
		var VisionBridgeForm = class {
			...
			plan(section) {
				...
				if (key !== void 0 && key.trim() !== "") writes.push({
					field: "apiKey",
					kind: "set",
					value: key.trim()
				});
